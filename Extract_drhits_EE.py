...
class Extract:
    ...
    def read(self, N=None):
        ...
        '''for i in range(0,len(target),chunk_size):
            lf_chunk=localfeat(HitsEta[i:i+chunk_size],HitsPhi[i:i+chunk_size],HitsEn[i:i+chunk_size])
            df_chunk = detfeat(HitsiEta[i:i+chunk_size],HitsiPhi[i:i+chunk_size],HitsEn[i:i+chunk_size])
            print("\tBuilding features took %f seconds" % (time() - t0))
            t0=time()
            result[f"lf_chunk{i}"] = torchify(lf_chunk)
            result[f"df_chunk{i}"] = torchify(df_chunk)
            print("\tTorchifying took %f seconds" % (time() - t0))
            with open(f"%s/lf_chunk{i}.pickle" % (self.outfolder), "wb") as f0:
            	torch.save(result[f"lf_chunk{i}"], f0, pickle_protocol=4)
            with open(f"%s/df_chunk{i}.pickle" % (self.outfolder), "wb") as f0:
            	torch.save(result[f"df_chunk{i}"], f0, pickle_protocol=4)'''
