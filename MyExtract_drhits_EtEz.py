...
'''
def cartfeat(x,y,z,E1,E2,flag) :
	return ak.concatenate((x[:, :, None], y[:, :, None], z[:, :, None], E1[:, :, None],E2[:, :, None],flag[:,:,None]),-1,)
...
class Extract:
    ...
    def read(self, N=None):
        ...
        for start in range(0,len(target),chunksize):
            ...
            HitsEta = ak.concatenate((arrs["RawHit_Eta_Pho1"][start:stop],arrs["RawHit_Eta_Pho2"][start:stop]),axis=1)

            Hit_theta=2 * np.arctan(np.exp(-HitsEta))
            Hit_sin_theta =np.sin(Hit_theta)
            Hit_abs_cos_theta =abs(np.cos(Hit_theta))
            HitsEn1=Hit_sin_theta*HitsEn
            HitsEn2 = Hit_abs_cos_theta*HitsEn
            ...
            HitES_theta = 2 * np.arctan(np.exp(-HitsESEta))
            HitES_sin_theta = np.sin(HitES_theta)
            HitES_abs_cons_theta = abs(np.cos(HitES_theta))
            HitsESEn1 = HitES_sin_theta*HitsESEn
            HitsESEn2 = HitES_abs_cons_theta*HitsESEn
            ...
            HitsZ = rescale(HitsZ,Z_Min,Z_Max)
            HitsEn = rescale(HitsEn,ECAL_Min,ECAL_Max)
            HitsEn1 = rescale(HitsEn1,ECAL_Min,ECAL_Max)
            HitsEn2 = rescale(HitsEn2,ECAL_Min,ECAL_Max)
            #Hitnoise=rescale(Hitnoise,Noise_Min,Noise_Max)
            ...
            HitsESZ = rescale(HitsESZ,ES_Zmin,ES_Zmax)
            HitsESEn = rescale(HitsESEn,ES_Min,ES_Max)
            HitsESEn1 = rescale(HitsESEn1,ES_Min,ES_Max)
            HitsESEn2 = rescale(HitsESEn2,ES_Min,ES_Max)
            #HitESnoise=HitsESEn*0
            ...
            HitsZ = ak.concatenate((HitsZ,HitsESZ),axis=-1)
            #HitsEn = ak.concatenate((HitsEn,HitsESEn),axis=-1)
            HitsEn1 = ak.concatenate((HitsEn1,HitsESEn1),axis=-1)
            HitsEn2 = ak.concatenate((HitsEn2,HitsESEn2),axis=-1)
            #HitsEta=ak.concatenate((HitsEta,HitsESEta),axis=-1)
            #HitsPhi=ak.concatenate((HitsPhi,HitsESPhi),axis=-1)
            #Hitnoise=ak.concatenate((Hitnoise,HitESnoise),axis=-1)'''
            flg = ak.concatenate((EEflg,ESflg),axis=-1)

            #cf=cartfeat(HitsX[start:stop],HitsY[start:stop],HitsZ[start:stop],HitsEn[start:stop],Hitnoise[start:stop],flg[start:stop])
            #cf=cartfeat(HitsX,HitsY,HitsZ,HitsEn,Hitnoise,flg)
            ...
            cf=cartfeat(HitsX,HitsY,HitsZ,HitsEn1,HitsEn2,flg)
